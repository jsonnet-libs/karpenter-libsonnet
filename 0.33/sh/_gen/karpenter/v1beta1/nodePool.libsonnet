{
  ...
  '#spec':: d.obj(help='"NodePoolSpec is the top level nodepool specification. Nodepools launch nodes in response to pods that are unschedulable. A single nodepool is capable of managing a diverse set of nodes. Node properties are determined from a combination of nodepool and pod scheduling constraints."'),
  spec: {
    '#disruption':: d.obj(help="\"Disruption contains the parameters that relate to Karpenter's disruption logic\""),
    disruption: {
      '#withConsolidateAfter':: d.fn(help='"ConsolidateAfter is the duration the controller will wait before attempting to terminate nodes that are underutilized. Refer to ConsolidationPolicy for how underutilization is considered."', args=[d.arg(name='consolidateAfter', type=d.T.string)]),
      withConsolidateAfter(consolidateAfter): { spec+: { disruption+: { consolidateAfter: consolidateAfter } } },
      '#withConsolidationPolicy':: d.fn(help='"ConsolidationPolicy describes which nodes Karpenter can disrupt through its consolidation algorithm. This policy defaults to \\"WhenUnderutilized\\" if not specified"', args=[d.arg(name='consolidationPolicy', type=d.T.string)]),
      withConsolidationPolicy(consolidationPolicy): { spec+: { disruption+: { consolidationPolicy: consolidationPolicy } } },
      '#withExpireAfter':: d.fn(help='"ExpireAfter is the duration the controller will wait before terminating a node, measured from when the node is created. This is useful to implement features like eventually consistent node upgrade, memory leak protection, and disruption testing."', args=[d.arg(name='expireAfter', type=d.T.string)]),
      withExpireAfter(expireAfter): { spec+: { disruption+: { expireAfter: expireAfter } } },
    },
    '#template':: d.obj(help='"Template contains the template of possibilities for the provisioning logic to launch a NodeClaim with. NodeClaims launched from this NodePool will often be further constrained than the template specifies."'),
    template: {
      '#metadata':: d.obj(help=''),
      metadata: {
    ...
    '#withWeight':: d.fn(help='"Weight is the priority given to the nodepool during scheduling. A higher numerical weight indicates that this nodepool will be ordered ahead of other nodepools with lower weights. A nodepool with no weight will be treated as if it is a nodepool with a weight of 0."', args=[d.arg(name='weight', type=d.T.integer)]),
